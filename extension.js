...
function activate(context) {
  ...
  let copySelectedCommand = vscode.commands.registerCommand(
    "fileCopier.copySelectedFiles",
    async function (uri, allUris) {
...
async function copySelectedItems(selectedUris) {
  ...
  try {
    ...
    // Remove duplicates
    allFiles = [...new Set(allFiles.map((f) => f.fsPath))].map((f) =>
      vscode.Uri.file(f)
    );
    ...
    vscode.window.showInformationMessage(
      `✓ Copied ${copiedCount} files from ${itemCount} selected ${itemWord} to clipboard!`
    );
  } catch (error) {
...
async function copyFiles() {
  ...
  try {
    ...
    await vscode.env.clipboard.writeText(output);
    vscode.window.showInformationMessage(
      `✓ Copied ${copiedCount} files from workspace to clipboard!`
    );
  } catch (error) {
    vscode.window.showErrorMessage(`Error: ${error.message}`);
  }
